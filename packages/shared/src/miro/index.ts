import { safeValidateCanvasDocument } from "../canvas/index.js";
import { UnsafeCanvasSpecError } from "../errors/index.js";
import {
...
export function generateMiroExportSpec({
  ...
  const validation = safeValidateCanvasDocument(canvas);
  if (!validation.ok) {
    throw new UnsafeCanvasSpecError(validation.errors.join("; "));
  }

  const validCanvas = validation.data;
  ...
  if (!sourceBlock || sourceBlock.type !== "SourceMethodBlock") {
    throw new UnsafeCanvasSpecError("Miro export requires SourceMethodBlock.");
  }
