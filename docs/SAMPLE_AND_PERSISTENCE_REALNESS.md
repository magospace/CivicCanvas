...
| Seed prompt examples | `data/prompt-examples.json` loaded by `apps/web/lib/data.ts` and passed into `/explore` | Fixture-backed demo guidance. | Prompt chips are governed examples that share the same validated source as deterministic/OpenAI fallback suggestions. | Do not treat prompt examples as hidden backend records or arbitrary prompt coverage. | `pnpm test -- apps/web/test/prompt-examples.test.ts`, `pnpm data:realism:json` |
