...
test("demo readiness route shows public release boundaries", async ({ page }) => {
  ...
  await expect(page.getByText(/No-key OpenAI mode uses deterministic guided suggestions/i)).toBeVisible();
  await expect(page.getByText(/default local demo path/i)).toBeVisible();
