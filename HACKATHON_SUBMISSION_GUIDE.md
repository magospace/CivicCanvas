...
## Provider And Media Proof Boundaries

- Default local demo path: no provider keys, no live OpenAI calls, no Fal/media calls, deterministic prompt parsing, and approved sample/live-fallback data paths.
- OpenAI proof: optional, server-side only, no-spend by default via `pnpm provider:openai:smoke:json`; live proof requires `RUN_LIVE_OPENAI_SMOKE=1` plus a server-side key and makes one minimal structured-output readiness call.
- Fal/media proof: optional script-only proof via `pnpm media:fal:smoke:json`; live proof requires `RUN_LIVE_FAL_SMOKE=1` plus a server-side key. Normal dashboard generation does not create image/video artifacts.
- What to say in the Loom: "CivicCanvas dashboards are generated by deterministic governed TypeScript paths. Optional providers can assist or prove readiness when gated, but they do not generate dashboard code, SQL, media, hidden fields, or public-data coverage claims."
